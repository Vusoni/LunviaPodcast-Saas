...
const SOCIAL_SYSTEM_PROMPT =
  "You are a world-class social media strategist specializing in creating viral content across all major platforms. For any content I provide, generate platform-specific posts that maximize engagement, reach, and audience growth. Tailor each post to the platform’s tone, audience expectations, and best practices. For each platform, provide 3–5 variations, include suggested formatting (length, paragraphs, emojis if relevant), and a 1–2 sentence rationale explaining why each variation will perform well. Focus on actionable, copy-ready posts that are unique to each platform and optimized for measurable results.";
...
export async function generateSocialMediaPosts(
  ...
  try {
    ...
    const socialPosts = content
      ? SocialMediaPostsSchema.parse(JSON.parse(content))
      : {
    ...
    return {
      twitter: "Error generating social post. Check logs for details. ",
      linkedin: "Error generating social post. Check logs for details. ",
      instagram: "Error generating social post. Check logs for details. ",
      tiktok: "Error generating social post. Check logs for details.",
      youtube: "Error generating social post. Check logs for details.",
      facebook: "Error generating social post. Check logs for details.",
    };
  }
}
