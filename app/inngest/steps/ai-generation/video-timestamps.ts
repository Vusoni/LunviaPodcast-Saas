...
export async function generateYouTubeTimestamps(
  ...
  const prompt = `You are a YouTube content optimization expert. Create SHORT, CATCHY CHAPTER TITLES for a video.

CRITICAL INSTRUCTIONS:
- Do NOT copy transcript text
- Do NOT write full sentences
- Titles must be 3–6 words only
- Think of these as chapter headings, not subtitles
- Return ONLY valid JSON in the format provided

I have ${
    chapterData.length
  } chapters with timestamps. For each chapter, create a SHORT, ENGAGING TITLE.
...
✓ "Introduction to N8N Automation" (descriptive, 5 words)
✓ "Setting Up Your Account" (action-oriented, 4 words)
✓ "Telegram Bot Creation" (clear, 3 words)
✓ "Building Multi-Agent Workflows" (technical, 3 words)
✓ "Testing the News Agent" (specific feature, 4 words)

EXAMPLES OF BAD TITLES:
✗ "Today we are diving into n8n one of the most underrated" (transcript excerpt)
✗ "One click setup with n8n com allows you to get" (full sentence)
✗ "So we want to give Sarah some instructions so she knows" (conversational)

RETURN FORMAT:
{
  "titles": [
    {"index": 0, "title": "Example Title One"},
    {"index": 1, "title": "Example Title Two"},
    {"index": 2, "title": "Example Title Three"}
  ]
}

Remember: Create **TITLES**, not transcript excerpts. Focus on clarity, brevity, and catchiness.`;
      ...
      messages: [
        {
          role: "system",
          content: `
You are a YouTube content expert. Your task is to generate **SHORT, DESCRIPTIVE TITLES** for video chapters.

CRITICAL INSTRUCTIONS:
- Only create TITLES (e.g., "Introduction to AI")
- Do NOT copy transcript text or write full sentences
- Titles must be 3–6 words
- Think of these as chapter headings, not subtitles
- Always return **valid JSON**
- Focus on clarity, brevity, and catchiness
